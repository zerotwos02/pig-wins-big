import { Container, Graphics, Text, Sprite, Point, Texture, AnimatedSprite, } from 'pixi.js';
import { CELL, GRID_COLS, GRID_ROWS } from '@/game/config';
import { SFX } from '@/audio/sound-manager';
// ---------------- small SFX helper ----------------
async function sfx(name, opts) {
    try {
        await SFX.ready;
        // @ts-ignore
        SFX.play(name, opts || {});
...
function getWindowCenterIn(layer, viewport) {
...
function makeCellSprite(texKey, size = CELL * 0.9) {
...
function makeParticleSprite(texKey, size = CELL * 0.45) {
    const spr = Sprite.from(texKey);
    spr.anchor.set(0.5);
    const w = Math.max(1, spr.texture.width);
    const h = Math.max(1, spr.texture.height);
...
export async function impactRingAt(fxLayer, x, y) {
...
export async function popAmountAt(fxLayer, x, y, amount) {
    const txt = new Text({
        text: `+${amount.toLocaleString()}`,
        style: { fontSize: 22, fontWeight: '900', fill: 0xffffff },
    });
    txt.anchor?.set?.(0.5);
...
export async function pinkBurstAtCell(fxLayer, atIdx, opts) {
...
export async function coinBurstAtCell(fxLayer, atIdx, opts) {
    const { x, y } = idxToXY(atIdx);
    const count = opts?.count ?? 26;
...
export async function mixedBurstAtCell(fxLayer, atIdx, opts) {
    ...
    if (singleSfx) {
        try {
            await SFX.ready;
            SFX.play('spark_burst', { volume: 0.72 });
        }
        catch { }
    }
    const tasks = [];
...
// ---------------- value token ----------------
export async function showValueTokenAtCell(fxLayer, atIdx, amount, msVisible = 800) {
    const { x, y } = idxToXY(atIdx);
    ...
    const txt = new Text({
        text: `$${amount.toLocaleString()}`,
        style: { fontSize: 24, fontWeight: '900', fill: 0xffffff },
    });
    txt.anchor?.set?.(0.5);
    ...
    await new Promise((r) => setTimeout(r, msVisible));
    // fade out
    await (async () => {
...
export async function animateHammerAction(opts) {
    const { fxLayer, fromIdx, toIdx, smashed, newTotalUnderHammer, centerOnWindow = false, viewportForCenter, rotateDuringMove = true, showAmount = false, anticipation = true, impactBurst = { count: 28, size: 1.0 }, impactGate, speed = 0.75, onImpact, } = opts;
    const sp = Math.max(0.2, speed); // clamp
    const scaleDur = (ms) => Math.round(ms / sp);
    const target = centerOnWindow
        ? getWindowCenterIn(fxLayer, viewportForCenter)
        : new Point(...Object.values(idxToXY(toIdx)));
    const startP = centerOnWindow
        ? new Point(target.x + CELL * 1.6, target.y - CELL * 1.2)
        : new Point(...Object.values(idxToXY(fromIdx)));
    // ---- hammer sprite
    ...
    if (rotateDuringMove)
        hammer.rotation = baseAngle;
    fxLayer.addChild(hammer);
    // 0) Approach in two phases (A: 85% path, B: hover near target)
    const phaseA = new Point(lerp(startP.x, target.x, 0.85), lerp(startP.y, target.y, 0.85));
    await tween(scaleDur(220), (k) => {
        const t = easeOutCubic(k);
        hammer.position.set(lerp(startP.x, phaseA.x, t), lerp(startP.y, phaseA.y, t));
    });
    const hoverPos = new Point(lerp(phaseA.x, target.x, 0.06), lerp(phaseA.y, target.y, 0.06));
    await tween(scaleDur(140), (k) => {
        const t = easeOutCubic(k);
        hammer.position.set(lerp(phaseA.x, hoverPos.x, t), lerp(phaseA.y, hoverPos.y, t));
    });
    // 1) Optional anticipation pull-back
    if (anticipation) {
        const back = new Point(lerp(hoverPos.x, target.x, -0.24), lerp(hoverPos.y, target.y, -0.24));
        sfx('hammer_draw', { volume: 0.6 });
        await tween(scaleDur(160), (k) => {
            const t = easeOutCubic(k);
            hammer.position.set(lerp(hoverPos.x, back.x, t), lerp(hoverPos.y, back.y, t));
            hammer.scale.set(baseScaleX * (1.02 - 0.04 * t), baseScaleY * (1.02 - 0.04 * t));
            if (rotateDuringMove)
                hammer.rotation = baseAngle - (Math.PI / 14) * t; // small wind-up
        });
    }
    ...
    await tween(scaleDur(150), (k) => {
        const t = easeOutCubic(k);
        hammer.position.set(lerp(hammer.position.x, target.x, t), lerp(hammer.position.y, target.y, t));
        if (rotateDuringMove)
            hammer.rotation = baseAngle + (Math.PI / 9) * t; // swing into hit
    });
...
export function ensureHammerBadge(fxLayer, atIdx, currentTotal) {
    const p0 = idxToXY(atIdx);
    let badge = fxLayer.children.find((c) => c instanceof Text && Math.abs(c.x - p0.x) < 4 && Math.abs(c.y - (p0.y + CELL * 0.34)) < 6);
    if (!badge) {
        badge = new Text({ text: '', style: { fontSize: 18, fontWeight: '900', fill: 0xfff275 } });
...
function getFlipbookFrames(prefix, max = 32) {
    const frames = [];
    for (let i = 0; i < max; i++) {
        const key = `${prefix}_${String(i).padStart(3, '0')}`;
        const tex = Texture.from(key);
        const w = tex?.frame?.width ?? tex.width ?? 0;
        const h = tex?.frame?.height ?? tex.height ?? 0;
        if (!w || !h)
            break;
        frames.push(tex);
    }
...
export async function playPigExplosion(opts) {
    const { fxLayer, atIdx, prefix = 'pig_explode', fps = 24, withSparks = true, } = opts;
    ...
    if (frames.length > 0) {
        ...
        const w = frames[0]?.frame?.width ?? frames[0].width ?? 1;
        const h = frames[0]?.frame?.height ?? frames[0].height ?? 1;
