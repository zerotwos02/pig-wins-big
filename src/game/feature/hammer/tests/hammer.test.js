...
import { areAdjacent, fuseAdjacentHammers, actHammerOne, stepHammers, } from '@/game/feature/hammer/hammer';
...
describe('hammer logic', () => {
    ...
    it('actHammerOne smashes an adjacent locked pig and moves onto it', () => {
        const h = idx(2, 2);
        const pig = idx(2, 3);
        const states = new Map([[h, { total: 10 }]]);
        const locked = new Map([[pig, { amount: 90, sym: 'pig' }]]);
    ...
    it('actHammerOne roams when no adjacent pigs, avoiding prevIndex', () => {
        const h = idx(0, 0);
        const states = new Map([[h, { total: 0, prevIndex: idx(0, 1) }]]);
        const locked = new Map(); // none
    ...
    it('stepHammers: fuses neighbors, smashes pigs, re-keys moved states', () => {
        ...
        const locked = new Map([[pigR, { amount: 50, sym: 'pig' }]]);
