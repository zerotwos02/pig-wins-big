...
export function findPigCells(grid, cols = GRID_COLS, rows = GRID_ROWS) {
    const out = [];
    const n = cols * rows;
    for (let i = 0; i < n; i++) {
        if (isPigKey(grid[i]))
...
export function findHammerCells(grid, cols = GRID_COLS, rows = GRID_ROWS) {
    const out = [];
    const n = cols * rows;
    for (let i = 0; i < n; i++) {
...
export function hasPigOnEveryReel(grid, cols = GRID_COLS, rows = GRID_ROWS) {
    for (let c = 0; c < cols; c++) {
        let found = false;
        for (let r = 0; r < rows; r++) {
            if (isPigKey(grid[r * cols + c])) {
                found = true;
                break;
            }
        }
...
export function neighborsOf(idx, cols = GRID_COLS, rows = GRID_ROWS) {
