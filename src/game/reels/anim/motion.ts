// src/game/reels/anim/motion.ts
import { Container } from 'pixi.js';
import { BlurFilter } from '@pixi/filter-blur';
import { tween, TweenHandle } from '@/anim/tween';
import { easeInOutCubic, easeOutBack, easeOutExpo } from '@/anim/easings';

type MotionOpts = {
  // ⬇️ NEW: pre-kick config for smoother start
  preKickPx?: number;           // nudge distance upwards (px). Default 12
  preKickMs?: number;           // total duration of pre-kick cycle. Default 200
  preKickStaggerMs?: number;    // ripple across columns. Default 40
  preKickOvershootRatio?: number; // how much it dips below zero after coming back (0..1). Default 0.22

  rampFrom?: number;     // speed scale at start
  rampTo?: number;       // steady speed scale
  rampMs?: number;       // ms for ramp-up
  decelTo?: number;      // target speed scale before first stop
  decelMs?: number;      // ms for global decel
  blurMaxY?: number;     // max vertical blur when speed=1
  bouncePx?: number;     // column settle bounce in px
  bounceMs?: number;     // ms for bounce
};
...
export class ReelsMotion {
  ...
  private setNudge: (col: number, y: number) => void;

  // ⬇️ NEW: separate pre-kick nudges (set from outside via ctor arg)
  private setStartNudge: (col: number, y: number) => void;

  private blur = new BlurFilter();
  private blurMaxY: number;

  private rampTween?: TweenHandle;
  private decelTween?: TweenHandle;
  private bounceTweens: (TweenHandle | null)[] = [];
  private preKickTweens: (TweenHandle | null)[] = [];

  private speed = 1;

  private cfg: Required<MotionOpts> = {
  ...
  constructor(args: {
    stripRoot: Container;
    colCount: number;
    setSpeed: (v: number) => void;              // calls setSpinSpeedScale
    setNudge: (col: number, y: number) => void; // adds small Y offset to a column (settle)
    setStartNudge: (col: number, y: number) => void; // ⬅️ NEW: pre-kick nudge setter
    options?: MotionOpts;
  }) {
    ...
    if (args.options) Object.assign(this.cfg, args.options);

    this.blurMaxY = this.cfg.blurMaxY;
    this.blur.quality = 3;
    this.blur.blurX = 0;
    this.blur.blurY = 0;
    (this.strip as any).filters = [this.blur];
  ...
  onRequestStop(immediate: boolean) {
    if (immediate) {
      this.killMoveTweens();
  ...
  onAllStopped() {
    ...
    tween({
      ...
      onComplete: () => {
        ...
        (this.strip as any).filters = null;
      },
    });
  }
  ...
  private runPreKick(onComplete: () => void) {
            ...
            this.preKickTweens[c] = tween({
              ...
              onComplete: () => {
                ...
                this.preKickTweens[c] = tween({
                  ...
                  ease: easeInOutCubic,
                  onUpdate: (v) => this.setStartNudge(c, v),
                  onComplete: () => { if (++done === this.colCount) onComplete(); },
                });
              },
            });
          },
