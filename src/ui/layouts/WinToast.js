...
function easeOutBack(t, s = 1.70158) { return 1 + (s + 1) * Math.pow(t - 1, 3) + s * Math.pow(t - 1, 2); }
...
export class WinToast extends Container {
    constructor() {
        ...
        this.coins = [];
        this.lastEmitAt = 0; // ms wall time
    ...
    show(amount, stake) {
        ...
        const barColor = tierIdx >= 2 ? GOLD_RICH :
            tierIdx === 1 ? GOLD_DEEP :
                GOLD_SOFT;
        ...
        return new Promise((resolve) => {
            const step = () => {
                ...
                // fade out
                    ...
                    if (confettiStarted)
                        this.updateConfetti(1 / 60, a * 0.6);
                }
                // done
                else {
                    this.visible = false;
    ...
    emitCoins(now, finalAmount) {
        if (!this.countingPhase)
            return;
