...
import { Container, Graphics, NineSliceSprite, Sprite, Text, Texture, Rectangle, } from 'pixi.js';
...
export class InfoModal extends Container {
    ...
    open() {
        SFX.ready.then(() => {
            ...
            if ('playLoop' in SFX)
                SFX.playLoop('info_loop', 0.35);
            else
                SFX.play?.('info_loop', { loop: true, singleInstance: true, volume: 0.35 });
        });
        this.visible = true;
        this.animateTo(1);
    }
    close() {
        SFX.ready.then(() => {
            ...
            if ('resume' in SFX)
                ...
                SFX.play?.(BGM_KEY, { loop: true, volume: 0.25 });
        });
    ...
    makePage1() {
        ...
        const body = this.makeBody('Wild substitutes for all icons except Pig icons.\n' +
    ...
    makePage2() {
        ...
        const hIcon = new Text('ICON', { fill: 0xffffff, fontSize: 16, fontWeight: '800' });
        const h3 = new Text('×3', { fill: 0xffffff, fontSize: 16, fontWeight: '800' });
        const h4 = new Text('×4', { fill: 0xffffff, fontSize: 16, fontWeight: '800' });
        const h5 = new Text('×5', { fill: 0xffffff, fontSize: 16, fontWeight: '800' });
        ...
        for (const [key, label, p3, p4, p5] of PAYTABLE) {
            ...
            const name = new Text(label, { fill: 0xffffff, fontSize: 16, fontWeight: '600' });
            const t3 = new Text(p3.toFixed(2), { fill: 0xffffff, fontSize: 16 });
            const t4 = new Text(p4.toFixed(2), { fill: 0xffffff, fontSize: 16 });
            const t5 = new Text(p5.toFixed(2), { fill: 0xffffff, fontSize: 16 });
    ...
    makePage3() {
        ...
        const body = this.makeBody('Use the – / + buttons to select bet amounts: 10, 20, 50, 100, 200, 500 (wraps at ends).\n' +
    ...
    makeCloseChip(label, r) {
        const c = new Container();
        const g = new Graphics()
            .circle(0, 0, r)
            .fill({ color: PURPLE, alpha: 1 })
            .stroke({ width: 3, color: YELLOW, alpha: 1 });
        const t = new Text({ text: label, style: { fill: 0xffffff, fontSize: 18, fontWeight: '900', align: 'center' } });
        t.anchor?.set?.(0.5);
        c.addChild(g, t);
        c.eventMode = 'static';
        c.cursor = 'pointer';
