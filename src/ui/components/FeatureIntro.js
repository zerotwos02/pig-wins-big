...
export class FeatureIntroModal extends Container {
    ...
    async present(opts) {
        const { spins, title = "CONGRATULATIONS", subtitle = "LOCK & WIN TRIGGERED", buttonLabel = "START", onShownSfx = "onShown", onClickSfx = "ui_click", } = opts;
        ...
        try {
            await SFX.bgmStop('bg_music', 160);
        }
        catch { }
        // pop-in animation
        this.alpha = 0;
        ...
        await new Promise((resolve) => {
            ...
            this.btn.once("pointertap", async () => {
                try {
                    ...
                    if (SFX.has('bg_lockandwin')) {
                        try {
                            SFX.stop('bg_lockandwin');
                        }
                        catch { }
                        SFX.fade('bg_lockandwin', 0, 0, 0);
                        SFX.play('bg_lockandwin'); // keep running during the feature
                        SFX.fade('bg_lockandwin', 0, 0.28, 160);
                    }
