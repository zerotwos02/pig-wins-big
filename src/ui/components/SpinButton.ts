...
type ButtonMode = 'spin' | 'stop' | 'disabled';

export class SpinButton extends Container {
  private btn: Sprite;
  private _mode: ButtonMode = 'spin';
  private _pressed = false;
  private texSpin: Texture;
  private texStop: Texture;

  // hook these from outside
  onSpin?: () => void;
  onStop?: () => void;
  ...
  // legacy alias
  onClick(fn: () => void) { this.onSpin = fn; }

  setMode(m: ButtonMode) {
  ...
  setEnabled(v: boolean) { this.setMode(v ? (this._mode === 'disabled' ? 'spin' : this._mode) : 'disabled'); }

  private handleClick() {
    if (this._mode === 'disabled') return;
    if (this._mode === 'spin') this.onSpin?.();
    else if (this._mode === 'stop') this.onStop?.();
  }
