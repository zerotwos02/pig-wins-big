...
export class gameScene extends Container {
    ...
    startSpin() {
        const s = getState();
        if (s.isSpinning || s.balance < s.stake)
            return;
        setSpinning(true);
        setWin(0);
        debitStake();
    ...
    sceneFlash(color = 0xFFD479, alpha = 0.55, fadeMs = 240) {
        ...
        if (this.fxRafFlash)
            cancelAnimationFrame(this.fxRafFlash);
    ...
    screenShake(intensityPx = 6, ms = 260) {
        if (this.fxRafShake)
            cancelAnimationFrame(this.fxRafShake);
    ...
    async awaitAndSettle() {
        try {
            ...
            if (this._forceHammerOnce) {
                ...
                if (!useFreeFlight)
                    this.reels.setCellAlpha(neighbor, 0);
                try {
                    ...
                    if (!useFreeFlight)
                        this.reels.setCellAlpha(neighbor, 1);
