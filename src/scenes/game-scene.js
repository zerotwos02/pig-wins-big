...
import { Container, Sprite, Texture, NineSliceSprite, Graphics } from 'pixi.js';
...
// ──────────────────────────────────────────────────────────────────────────────
const AUTO_STOP_MS = 1100;   // fallback stop if user doesn't press STOP
const HIGHLIGHT_MS = 900;    // duration to keep highlight rectangles

export class gameScene extends Container {
    constructor() {
        super(...arguments);
        this.grid = new Container();     // local container for reels + FX (masked)
        // Clips reels + FX to the panel’s inner window
        this.gridMask = new Graphics();  // dynamic mask rebuilt in layout()
        this._bgStarted = false;         // start BGM once on first interaction
        this.hud = new HUD();
        this.reels = new Reels();
        this.winToast = new WinToast();
        ...
        // Keyboard-only developer shortcuts (Spin handled in HUD's SpinButton)
        ...
        // Ticker callback (advance reels motion)
        this.onTick = (t) => {
            this.reels.update(t.deltaMS);
        };
    }

    // One-time scene setup (called after assets are loaded)
    async init() {
        ...
        // Panel frame around the reels (9-slice)
        ...
        this.hud.spin.onSpin = async () => {
            await SFX.ready;
            if (!this._bgStarted) {
                SFX.play('bg_music');     // start looped BGM on first spin
        ...
        // Full-scene flash overlay (used on big wins/FX)
        this.fxFlash = new Graphics();
        this.fxFlash.zIndex = 1500;
        this.fxFlash.visible = false;
        this.addChild(this.fxFlash);

        // Start BGM on first pointer (mobile autoplay policy friendly)
    ...
    // Responsive layout: scales/positions background, panel, grid, HUD, and mask.
    layout() {
        ...
        const target = Math.min(W, H) * 0.8;        // fit reels within 80% of shortest axis
        ...
        // HUD reel rect (for aligning controls/labels with the window)
        ...
        // Info & audio positions (top-right cluster)
        const r = 18, PAD = 16;
        ...
        // Flash overlay reset to full-screen transparent
        if (this.fxFlash) {
    ...
    startSpin() {
        ...
        // Call backend spin; on failure, keep current visible grid and zero win.
        this.pendingSpin = spin(getState().stake).catch((e) => {
            console.error('spin() failed:', e);
            return { grid: this.reels.getVisibleGrid(), win: 0 };
        });

        // Arm auto stop in case the user doesn't press STOP
        this.armAutoStop();
    }
    ...
    // Quick full-screen flash that fades out (used for wins/impacts)
    sceneFlash(color = 0xFFD479, alpha = 0.55, fadeMs = 240) {
    ...
    // Simple screen shake with linear damping
    screenShake(intensityPx = 6, ms = 260) {
    ...
    // Build a centered sprite sized to a cell (respecting texture dimensions)
    makeCellSprite(texKey, size = CELL * 0.9) {
    ...
    // Local visual: overlay a pig sprite at idx and squash/fade it quickly
    async playLocalPigSquash(idx, texKey) {
    ...
    async awaitAndSettle() {
        try {
            ...
            if (DEV_FORCE_LW || this._forceLWOnce) {
                ...
                const setCellKey = this.reels.setCellKey;
            ...
            // Collect cells to highlight for all wins
            const hlSet = new Set();
            for (const w of wins) for (const i of w.indices) hlSet.add(i);

            // Optional: hammer awards calculated in base game
            ...
            // Commit winnings and feedback for base game
            setWin(grand);
    ...
    // Cleanup listeners and animation frames when the scene is destroyed
    destroy(options) {
