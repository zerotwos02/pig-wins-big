...
import { Symbol as PbSymbol, SpinResponseSchema, GridSchema, BaseWinSchema, PigFeatureSchema, } from "../gen/game_pb";
import { GRID_COLS as COLS, GRID_ROWS as ROWS, PIG_TRIGGER_COUNT, } from "../game/config";
...
const rc = (i) => ({ r: (i / COLS) | 0, c: i % COLS });
...
// ───────────────────────────────────────────────────────────────
function powerPays(cells, stake) {
    const baseWins = [];
...
function hammerInstantCollect(cells, stake) {
    let sum = 0;
    for (let i = 0; i < cells.length; i++) {
        if (cells[i] !== PbSymbol.SYM_HAMMER)
            continue;
...
// ───────────────────────────────────────────────────────────────
export async function mockSpin(stake) {
    // 1) random 5×5 grid
    ...
    const feature = pigCount >= PIG_TRIGGER_COUNT
